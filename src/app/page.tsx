...
export default function WagmiEliteTerminal() {
  ...
  const analyze = async () => {
    ...
    setLoading(true);
    try {
      // استخدام RPC مستقر وموثوق
      const connection = new Connection("https://api.mainnet-beta.solana.com", "confirmed");
      const key = new PublicKey(address.trim());
      const balance = await connection.getBalance(key);
      const sol = balance / 1_000_000_000;

      // منطق الحاملين والرتبة (Holders Logic)
      let currentStatus = sol >= 100 ? "SOLANA WHALE" : sol >= 10 ? "ALPHA TRADER" : "WAGMI SOLDIER";
      let assetLabel = "SOL";

      // فحص إذا كان العنوان يحتوي على troll أو رصيد ضخم لعملة معينة (محاكاة احترافية)
  ...
  return (
    <div className="min-h-screen bg-[#000] text-white font-sans selection:bg-cyan-500 overflow-x-hidden">
      ...
      <section className="relative z-10 min-h-screen flex flex-col items-center justify-center p-6">
        ...
        >
          {/* LOGO FIX: Clean, Sharp, No Overlap */}
          <h1 className="text-7xl md:text-[14rem] font-[1000] italic tracking-tighter leading-none bg-gradient-to-b from-white via-white to-gray-700 bg-clip-text text-transparent drop-shadow-[0_0_50px_rgba(255,255,255,0.1)]">
            WAGMI
          </h1>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-xl px-4 space-y-6">
          ...
          >
            {loading ? "SCANNING CORE NETWORK..." : "INITIALIZE SCAN"}
          </button>
        </motion.div>
      </section>
      ...
      <AnimatePresence>
        {data && (
          <section id="result-view" className="relative z-10 py-32 flex flex-col items-center bg-gradient-to-b from-transparent via-cyan-950/10 to-transparent">
                ...
                <div ref={cardRef} className="relative w-full h-full bg-[#050505] rounded-[2.3rem] md:rounded-[3.3rem] p-8 md:p-12 overflow-hidden flex flex-col justify-between z-10">
                  ...
                  <div className="flex items-center gap-8 text-left">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-mono text-white/40 tracking-[0.6em] uppercase mt-2 italic font-bold tracking-widest">ASSET_RESERVE: {data.asset}</p>
                    </div>
                  </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
          ...
          <div className="flex flex-col items-center md:items-end gap-6">
            ...
            <p className="text-[10px] font-mono tracking-[0.8em] text-gray-600 uppercase">
              Designed & Developed by <span className="text-white font-black italic">Bader Alkorgli</span>
            </p>
          </div>
        </div>
      </footer>
