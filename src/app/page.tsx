...
// بيانات رادار الحيتان الافتراضية
...
export default function WagmiWhaleProtocolFull() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      alert("Invalid Signal! Science requires precision!");
    } finally {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-[#010101] text-white flex flex-col items-center p-4 md:p-10 font-sans overflow-hidden relative">
      ...
      {/* 2. شخصية Senku (GIF) تسبح في كامل الخلفية */}
      <motion.div 
      ...
      >
        <div className="relative">
          <img 
            src="/senku.gif" 
            alt="Senku"
            className="w-[280px] md:w-[550px] opacity-50 filter contrast-150 brightness-110 drop-shadow-[0_0_60px_rgba(59,130,246,0.3)]"
          ...
          />
          <motion.div className="absolute -top-5 -right-5 bg-black/70 border border-blue-500/30 p-2 rounded-xl text-[9px] font-mono text-blue-400 backdrop-blur-md whitespace-nowrap">
            {loading ? "10 Billion Percent Success..." : "Science is the rules of the universe."}
          </motion.div>
        </div>
      </motion.div>

      {/* 3. عناصر التحكم العلوية */}
      <button 
      ...
      <div className="relative z-[70] flex bg-white/5 border border-white/10 p-1 rounded-2xl mb-14 backdrop-blur-2xl">
        <button 
          ...
          className={`px-8 py-3 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all ${activeTab === 'scan' ? 'bg-blue-600 text-white' : 'text-white/30 hover:text-white'}`}
        >
          Identity Scan
        </button>
        <button 
          onClick={() => setActiveTab('radar')} 
          className={`px-8 py-3 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all ${activeTab === 'radar' ? 'bg-blue-600 text-white' : 'text-white/30 hover:text-white'}`}
        >
          Whale Radar
        </button>
      </div>

      {/* 5. المحتوى الرئيسي (أعلى من Senku) */}
        ...
        {activeTab === 'scan' ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full flex flex-col items-center">
            ...
            <div className="w-full max-w-md px-4 mb-16">
              ...
              <button 
                onClick={analyze} 
                className="w-full mt-5 py-5 bg-white text-black rounded-2xl font-black uppercase text-[10px] tracking-[0.5em] hover:bg-blue-600 hover:text-white transition-all active:scale-95 flex items-center justify-center gap-3"
              >
                {loading ? <Cpu className="animate-spin" size={16} /> : <Zap size={16} />} INITIATE_SCAN
              </button>
            </div>
            ...
            <AnimatePresence>
              {data && (
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full flex flex-col items-center pb-32 px-4">
                  <div ref={cardRef} className="relative w-full max-w-[480px] aspect-[1.6/1] bg-[#050505] border border-white/10 rounded-[2.5rem] p-10 flex flex-col justify-between overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)]">
                  ...
                  <button onClick={saveCard} className="mt-8 flex items-center gap-4 bg-white/5 border border-white/10 px-8 py-4 rounded-full font-black text-[9px] uppercase hover:bg-white hover:text-black transition-all">
                    SAVE DIGITAL IDENTITY <Download size={14} />
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full px-4 pb-48">
            <div className="flex items-center justify-between mb-12">
              ...
              <span className="text-[10px] font-mono text-blue-500 bg-blue-500/10 px-4 py-1.5 rounded-full border border-blue-500/20 font-black animate-pulse">LIVE_STREAMING</span>
            </div>

            <div className="grid gap-6">
              {MOCK_WHALE_DATA.map((whale) => (
                <div key={whale.id} className="bg-[#080808]/80 border border-white/10 p-6 md:p-8 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center gap-6 backdrop-blur-md">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-mono text-white/30 uppercase tracking-widest italic">${whale.usd} MARKET VALUE</p>
                    </div>
                  </div>
            ...
            <div className="mt-16 p-12 border border-white/5 rounded-[4rem] bg-gradient-to-br from-blue-600/5 to-transparent text-center">
              <Eye className="mx-auto mb-5 text-blue-500/40" size={36} />
              <h3 className="text-2xl font-[1000] italic text-white/80 uppercase">Sentiment: Bullish Accumulation</h3>
              <p className="text-[10px] font-mono text-white/20 mt-3 tracking-[0.4em] uppercase">Deep Ocean Logic v2.0</p>
            </div>
          </motion.div>
        )}
      </div>
      ...
      {/* Footer */}
      <footer className="mt-auto py-12 opacity-10 text-[8px] md:text-[10px] font-mono tracking-[1.5em] uppercase text-center w-full">
