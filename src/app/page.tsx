 ...
 * VERSION: V8.0 - INTEGRATED AGENT SYSTEM
 */
...
export default function SenkuUltimateProtocol() {
  ...
  const [agentMessage, setAgentMessage] = useState("SYSTEM READY. WAITING FOR NEURAL LINK...");
  ...
  useEffect(() => {
    if (loading) {
      setAgentMessage("PENETRATING SOLANA MAINNET NODES... PLEASE STAND BY.");
    } else if (data) {
      setAgentMessage(`LINK ESTABLISHED. BRAIN POWER: ${intelligenceScore} IQ. ASSET VELOCITY IS OPTIMAL.`);
    } else {
      const tips = {
        'scan': "GREETINGS BADER. READY TO DECODE ANY SOLANA ADDRESS?",
        'rug shield': "RUG SHIELD ACTIVE. I WILL DETECT ANY MALICIOUS CODE IN THE CONTRACT.",
        'radar': "MONITORING INSTITUTIONAL FLOW. THE WHALES ARE ACTIVE TODAY.",
        'hall of fame': "VIEWING THE ELITES. YOUR PROGRESS IS BEING TRACKED."
      };
      setAgentMessage(tips[activeTab as keyof typeof tips] || "SYSTEM OPERATIONAL.");
    }
  }, [activeTab, loading, data, intelligenceScore]);
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setIntelligenceScore(Math.floor(Math.random() * 40) + 80);
        ...
        setAgentMessage("ERROR: INVALID NEURAL COORDINATES. CHECK ADDRESS.");
    ...
    <div className="min-h-screen bg-[#020617] text-white flex flex-col items-center p-4 md:p-8 font-sans overflow-hidden relative selection:bg-green-500/30">
      ...
      <main className="relative z-10 w-full max-w-6xl flex flex-col items-center flex-grow justify-center">
        ...
        <motion.div 
            ...
            className="w-full max-w-lg mb-6 px-4"
        >
            <div className="bg-green-500/5 border border-green-500/20 rounded-2xl p-4 flex items-center gap-4 backdrop-blur-md">
                <div className="w-10 h-10 rounded-full bg-green-500/20 flex items-center justify-center flex-shrink-0">
                    <BrainCircuit className="text-green-500 animate-pulse" size={20} />
                </div>
                <div>
                    <p className="text-[9px] font-black text-green-500 uppercase tracking-widest">Senku Intelligence Agent</p>
                    <p className="text-[11px] font-mono text-white/70 italic leading-tight">"{agentMessage}"</p>
                </div>
            </div>
        </motion.div>
        ...
        {activeTab === 'scan' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full flex flex-col items-center">
            ...
            <div className="relative mb-4 group">
                <h1 className="text-[18vw] md:text-[13rem] font-[1000] italic tracking-tighter leading-none bg-gradient-to-b from-white via-white/80 to-green-500 bg-clip-text text-transparent select-none drop-shadow-[0_0_30px_rgba(34,197,94,0.3)] group-hover:drop-shadow-[0_0_50px_rgba(34,197,94,0.5)] transition-all duration-700">
                    SENKU
                </h1>
                <div className="absolute -bottom-2 right-4 bg-green-600 text-black px-2 py-0.5 text-[10px] font-black skew-x-[-12deg] tracking-tighter uppercase">Protocol V7.5</div>
            </div>

            <div className="w-full max-w-lg px-6 mb-16">
              <input className="w-full bg-slate-900/80 border border-white/10 rounded-2xl p-6 text-center outline-none focus:border-green-500 font-mono text-sm transition-all focus:bg-slate-900" placeholder="SOLANA_ADDRESS" value={address} onChange={(e) => setAddress(e.target.value)} />
              <button onClick={analyze} className="w-full mt-5 py-6 bg-white text-black rounded-2xl font-[1000] uppercase text-[11px] tracking-[0.5em] hover:bg-green-600 hover:text-white transition-all transform active:scale-95 shadow-xl">
                {loading ? "DECODING NEURAL PATHWAYS..." : "START NEURAL LINK"}
              </button>
            </div>
            {data && (
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="pb-32 px-4 w-full flex flex-col items-center gap-6">
                  ...
                  <motion.div onClick={() => setIsModalOpen(true)} className="relative cursor-pointer group w-full max-w-md bg-slate-900/40 border border-white/10 rounded-3xl p-6 flex items-center justify-between hover:bg-slate-800/50 transition-all">
        ...
        {activeTab === 'rug shield' && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-3xl px-6 pt-10 pb-40 text-center">
            <div className="mb-12 flex flex-col items-center">
                <div className="w-20 h-20 bg-green-500/10 rounded-3xl flex items-center justify-center mb-6 border border-green-500/20 shadow-[0_0_30px_rgba(34,197,94,0.2)]"><ShieldCheck size={40} className="text-green-500" /></div>
            ...
            <button onClick={analyzeRug} className="w-full py-6 bg-green-600 rounded-2xl font-[1000] text-[11px] uppercase tracking-[0.5em] shadow-lg active:scale-95 transition-all">{isAnalyzingRug ? "AUDITING NEURAL CONTRACTS..." : "CHECK SECURITY"}</button>
            {rugAnalysis && (
                <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                    <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-md"><span className="text-[10px] font-black text-green-500 uppercase">Safety Score</span><div className="text-6xl font-[1000] italic">{rugAnalysis.score}</div></div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-6 px-6 pt-10 pb-40">
            {[{ id: 'SENKU_PRIME', val: '50,000' }, { id: 'CHROME_X', val: '22,500' }].map((w, i) => (
              <div key={i} className="bg-slate-900/40 border border-white/10 p-10 rounded-[2.5rem] flex items-center gap-6 relative overflow-hidden group hover:border-green-500 transition-all">
                ...
                <div className="w-16 h-16 rounded-2xl bg-green-600 flex items-center justify-center font-[1000] text-3xl italic shadow-lg">#{i+1}</div>
