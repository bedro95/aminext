...
import { getRiskLevel, getRiskLabel, getRiskGradient, getRiskColor } from "@/lib/utils";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link
              href="/professions"
              className="hidden sm:flex items-center gap-1.5 text-xs font-semibold text-sky-600 dark:text-sky-400 hover:underline"
            >
                ...
                >
                  <Link href={`/profession/${profession.slug}`} className="group block">
                    <div className="flex items-center gap-3 p-3.5 rounded-xl border border-[var(--card-border)] bg-[var(--card)] hover:border-transparent hover:-translate-y-px hover:shadow-lg hover:shadow-black/5 dark:hover:shadow-black/20 transition-all duration-150">
                      ...
                      {/* Rank badge */}
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 text-xs font-black bg-gradient-to-br ${getRiskGradient(profession.automationScore)} text-white`}>
                        {i + 1}
                      </div>
                      ...
                      <div className="flex-shrink-0 text-right">
                        <span className={`text-lg font-black tabular-nums ${getRiskColor(profession.automationScore)}`}>
        ...
        <div>
          ...
          <div className="p-4 rounded-xl border border-sky-200/80 dark:border-sky-500/20 bg-gradient-to-br from-sky-50 to-indigo-50/50 dark:from-sky-500/8 dark:to-indigo-500/5">
            ...
            <p className="text-xs text-sky-600 dark:text-sky-300/80 leading-relaxed">
              ...
              the most automation-resistant through 2035.
            </p>
          </div>
        </div>
      </div>
